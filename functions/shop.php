...
<?php
/* 
/* @post_meta product_sales 商品销量(非手动添加)
*/

//创建订单和优惠码数据库
if ( !defined('ABSPATH') ) {exit;}
if(ot_get_option('shop_system')!='on') {return;}
...
//创建商城自定义文章类型，商城下每一篇文章即为一件商品
function create_store_post_type() {
	$store_slug = ot_get_option('shop_archives_slug','store');
...
function store_seo_info(){
	if ( get_post_type() == 'store'&&!is_singular() ){
		...
		if((taxonomy_exists('products_category')||taxonomy_exists('products_tag'))&&$id!=0){
			...
			$title = $term_name;
			$paged = get_query_var('paged'); if ( $paged > 1 ) $title .= sprintf(__(' - 第 %s 页 ','tinection'),$paged);
...
function custom_store_link( $link, $post = 0 ){
	$store_slug = ot_get_option('shop_archives_slug','store');
	$product_slug = ot_get_option('product_link_mode')=='post_name'?$post->post_name:$post->ID;
...
function custom_store_rewrites_init(){
	$store_slug = ot_get_option('shop_archives_slug','store');
	if(ot_get_option('product_link_mode')=='post_name'):
	add_rewrite_rule(
...
function get_user_order_records($product_id=0,$user_id=0,$success_orders=0){
	...
	if(is_user_logged_in()){
		...
		$prefix = $wpdb->prefix;
		if($product_id==0):
			if($success_orders==0){$orders=$wpdb->get_Results("select * from ".$prefix."tin_orders where user_id=".$id,'ARRAY_A');}else{$orders=$wpdb->get_Results("select * from ".$prefix."tin_orders where order_status=4 and user_id=".$id,'ARRAY_A');}
		else:
			if($success_orders==0){$orders=$wpdb->get_Results("select * from ".$prefix."tin_orders where user_id=".$id." and product_id=".$product_id,'ARRAY_A');}else{$orders=$wpdb->get_Results("select * from ".$prefix."tin_orders where order_status=4 and user_id=".$id." and product_id=".$product_id,'ARRAY_A');}
		endif;
		$record = $orders;
	}
	return $record;
...
function create_the_order(){
	...
	if (!wp_verify_nonce( trim($_POST['wp_nonce']), 'order-nonce' ) ){
		...
		if($currency==0){
			...
			//现金支付方式，首先插入数据库订单记录
			$insert = insert_order($_POST['product_id'],$_POST['order_name'],$price[5],$quantity,$cost_promoted,1,$order_note,$uid,$_POST['receive_name'],$_POST['receive_email'],$_POST['receive_address'],$_POST['receive_zip'],$_POST['receive_phone'],$_POST['receive_mobile'],$_POST['order_msg']);
			if($insert){
				$redirect = 1;
				$success = 1;
				$order_id = $insert;
...
function create_credit_recharge_order(){
	...
	if(!is_user_logged_in()){$msg='请先登录';}else{
		...
		if($product_id!=-4){$msg='系统发生错误，请刷新再试';}else{
...
if ( !defined( 'SHOP_URI' ) ) {
	define( 'SHOP_URI', get_template_directory().'/functions/shop.php' );
}
